    ...
    def __init__(self):
        self.config = {
            ...
            'ad_clicking': {
                ...
                'click_chance': random.uniform(0.05, 0.4),  # Ditingkatkan ke 5-40% untuk lebih efektif
                'max_clicks_per_session': random.randint(1, 5),  # Ditingkatkan ke 1-5 untuk lebih efektif
                'conservative_mode': True,
                'ad_types': ['google_adsense', 'google_vignette', 'google_discovery'],  # Based on sample files analysis
                ...
                'session_skip_chance': random.uniform(0.05, 0.2),  # Dikurangi ke 5-20% untuk lebih efektif
                ...
                'min_session_duration': random.uniform(5, 15),  # Minimal 5-15 detik session (reduced)
                'max_daily_clicks': random.randint(10, 25),  # Maksimal 10-25 klik per hari (increased)
            ...
            'article': {
                'default_urls': [
                    ...
                    'https://setiap.zonagamegratisan.com/dana-mudah-cair-tunaiku/',
                    'https://setiap.zonagamegratisan.com/pinjaman-online-di-tahun-2025-danamas/',

                    'https://cekmedia.com/pinjaman-online-langsung-cair-ke-rekening-tanpa-verifikasi-wajah-solusi-cepat-aman-di-2025-gendon/',
                    'https://cekmedia.com/7-apk-pinjaman-bunga-rendah-terbaik-terpercaya-2025-pilihan-aman-tips-pengajuan-dan-daftar-legal-ojk/',
                    'https://cekmedia.com/pinjaman-online-untuk-umkm-rekomendasi-aplikasi-terbaik-uji-langsung-dan-review-real-pengguna/',
                    
                    'https://pintar.cekmedia.com/rekomendasi-pinjaman-online-untuk-karyawan-gaji-umr-update-2025/',
                    'https://pintar.cekmedia.com/cara-menghindari-pinjaman-online-ilegal-dan-penipuan-digital-panduan-2025/',
                ...
                'referer_method': 'auto',  # 'auto', 'real', 'proxy', 'cdp', 'direct'
                'mix_direct_referer': True,  # Enable mix of direct and referer traffic
                'direct_ratio': 0.3,  # 30% direct, 70% referer (when mix enabled)
                'natural_pattern': True  # Use natural traffic patterns
            }
        }
...
class DetectionPatterns:
    ...
    def __init__(self):
        self.ad_detection_patterns = {
            'google_adsense': [
                # Priority 1: Filled ads only (most specific)
                'ins.adsbygoogle[data-ad-status="filled"]',
                'ins.adsbygoogle[data-adsbygoogle-status="done"][data-ad-status="filled"]',
                
                # Priority 2: Specific AdSense elements with client and slot
                'ins.adsbygoogle[data-ad-client][data-ad-slot]',
                'ins.adsbygoogle[data-ad-client="ca-pub-"][data-ad-slot]',
                
                # Priority 3: AdSense with specific format
                'ins.adsbygoogle[data-ad-format="auto"]',
                'ins.adsbygoogle[data-ad-format="rectangle"]',
                'ins.adsbygoogle[data-ad-format="banner"]',
                
                # Priority 4: Container divs with specific IDs (from samples)
                'div[id*="aswift"][id*="host"]',  # aswift_1_host, aswift_2_host, etc.
                'iframe[id*="aswift"][id*="aswift_"]',  # aswift_1, aswift_2, etc.
                
                # Priority 5: Auto-placed containers with ads
                'div.google-auto-placed ins.adsbygoogle',
                'div.ap_container ins.adsbygoogle'
            ],
            'google_adsense_exclude': [
                # Exclude unfilled ads (most important)
                'ins[data-ad-status="unfilled"]',
                'ins.adsbygoogle[data-ad-status="unfilled"]',
                'ins.adsbygoogle-noablate[data-ad-status="unfilled"]',
                
                # Exclude auto-placed ads that are unfilled
                'div.google-auto-placed ins[data-ad-status="unfilled"]',
                'div.ap_container ins[data-ad-status="unfilled"]',
                ...
                # Exclude ads with opacity 0
                'ins[style*="opacity: 0"]',
                'div[style*="opacity: 0"]',
                
                # Exclude ads with undefined dimensions
                'ins[style*="width: undefined"]',
                'ins[style*="height: undefined"]',
                
                # Exclude navigation elements that might be detected as ads
                'a[href*="/pinjaman-online"]',
                'a[href*="/dana-mudah-cair"]',
                'a[href*="/pinjaman"]',
                'a[href*="/kredit"]',
                'a[href*="/modal"]',
                'a[href*="/bantuan"]',
                ...
                'a[href*="/finansial"]',
                'a[href*="/keuangan"]',
                'a[href*="setiap.zonagamegratisan.com"]',
                'a[href*="zonagamegratisan.com"]',
                'a[href*="primecapitalaid.web.id"]',
                'a[href*="tokenomics.web.id"]',
                'a[href*="gradua.web.id"]',
                
                # Exclude related posts section
                '.idblog-related-post a',
                '.related-title + ul a',
                'h3.related-title + ul a',
                '.idblog-core a',
                '.idblog-related-post li a'
            ],
            'google_vignette': [
                # Based on actual analysis of vigennet.html
                
                # Auto-placed ads with noablate class (vignette ads)
                'ins.adsbygoogle.adsbygoogle-noablate',
                'ins.adsbygoogle-noablate',
                'ins[class*="adsbygoogle-noablate"]',
                
                # Auto-placed containers with vignette ads
                'div.google-auto-placed ins.adsbygoogle-noablate',
                'div.ap_container ins.adsbygoogle-noablate',
                
                # Vignette ads with specific attributes
                'ins.adsbygoogle-noablate[data-ad-format="auto"]',
                'ins.adsbygoogle-noablate[data-ad-client]',
                'ins.adsbygoogle-noablate[data-ad-slot]',
                
                # Vignette ads with status
                'ins.adsbygoogle-noablate[data-ad-status="filled"]',
                'ins.adsbygoogle-noablate[data-ad-status="unfilled"]',
                'ins.adsbygoogle-noablate[data-adsbygoogle-status="done"]'
            ],
            'google_discovery': [
                # Based on actual analysis of sample2.html
                
                # Related Topics Widget (found in sample2.html)
                '.goog-rtopics',
                'div.goog-rtopics',
                '[class*="goog-rtopics"]',
                'div[class*="goog-rtopics"][aria-label*="topics"]',
                'div[class*="goog-rtopics"][tabindex]',
                'div[class*="goog-rtopics"][aria-label*="related"]'
            ],
        }
        ...
        self.navigation_patterns = {
            'next': [
                ...
                # WordPress theme navigation (from sample.html analysis)
                '.idblog-related-post a',
                '.related-title + ul a',
                'h3.related-title + ul a',
                '.idblog-core a',
...
class AdClickingSystem:
    ...
    def __init__(self, driver, timing_system, mouse_simulator):
        ...
        self.session_start_time = time.time()  # Initialize session start time
        
    def detect_ads(self, page_source=None):
        """Detect ads pada current page with enhanced debugging"""
        detected_ads = []
        ...
        try:
            ...
            # Use SAME detection logic as referer mode (proven to work)
            self.logger.info(f"[DETECT] Starting ad detection with {len(self.patterns.ad_detection_patterns)} ad types")
            
            # Enhanced pre-detection wait for Direct mode
            self.logger.info("[DETECT] Pre-detection: Waiting for ads to load...")
            time.sleep(random.uniform(2, 4))
            
            # EXTRA wait for Direct mode detection
            self.logger.info("[DETECT] Additional Direct mode ad detection wait...")
            time.sleep(random.uniform(3, 6))  # Extra 3-6 seconds for Direct mode
            
            # Check different ad patterns
            for ad_type, selectors in self.patterns.ad_detection_patterns.items():
                # Skip exclude patterns
                if ad_type == 'google_adsense_exclude':
                    continue
                    
                self.logger.debug(f"[DETECT] Checking {ad_type} with {len(selectors)} selectors")
                ad_type_count = 0
                ...
                for selector in selectors:
                    try:
                        ...
                        if elements:
                            self.logger.debug(f"[DETECT] Found {len(elements)} elements with selector: {selector}")
                        
                        for element in elements:
                            if self._is_valid_ad_element(element):
                                # Log detail elemen yang terdeteksi
                                element_tag = element.tag_name
                                element_class = element.get_attribute('class') or 'no-class'
                                element_id = element.get_attribute('id') or 'no-id'
                                element_href = element.get_attribute('href') or 'no-href'
                                ...
                                self.logger.info(f"[DETECT] Found {ad_type} ad: {selector}")
                                self.logger.info(f"[DETECT]   Tag: {element_tag}, Class: {element_class[:50]}, ID: {element_id[:30]}")
                                self.logger.info(f"[DETECT]   Size: {ad_info['size']['width']}x{ad_info['size']['height']}, Position: ({ad_info['position']['x']}, {ad_info['position']['y']})")
                                self.logger.info(f"[DETECT]   Visible: {ad_info['is_visible']}, Click Prob: {ad_info['click_probability']:.3f}")
                                if element_href != 'no-href':
                                    self.logger.info(f"[DETECT]   Href: {element_href[:60]}...")
                                
                    except Exception as e:
                        self.logger.debug(f"[DETECT] Error with selector '{selector}': {e}")
                        continue
                
                if ad_type_count > 0:
                    self.logger.info(f"[DETECT] Found {ad_type_count} {ad_type} ads")
                else:
                    self.logger.debug(f"[DETECT] No {ad_type} ads found")
            
            self.logger.info(f"[DETECT] Total ads detected: {len(detected_ads)}")
            return detected_ads
            
        except Exception as e:
            self.logger.error(f"[DETECT] Error detecting ads: {e}")
            return []
    
    def _is_valid_ad_element(self, element):
        """Validate if element is a valid ad element with enhanced detection"""
        try:
            # Check for excluded ad patterns (based on sample analysis)
            if self._is_excluded_ad_element(element):
                self.logger.debug(f"[VALIDATE] Element excluded by filter")
                return False
            
            # Check if element is visible and has reasonable size
            if not element.is_displayed():
                self.logger.debug(f"[VALIDATE] Element not displayed")
                return False
            
            size = element.size
            if size['width'] < 30 or size['height'] < 30:  # Reduced minimum size
                self.logger.debug(f"[VALIDATE] Element too small: {size['width']}x{size['height']}")
                return False
            
            # Check if element is in viewport
            location = element.location
            if location['x'] < -100 or location['y'] < -100:  # Allow some margin
                self.logger.debug(f"[VALIDATE] Element outside viewport: ({location['x']}, {location['y']})")
                return False
            
            # STRICT VALIDATION: Hanya elemen dengan atribut iklan yang jelas
            ad_attributes = [
                'data-ad-client', 'data-ad-slot', 'data-ad-format', 'data-ad-layout',
                'data-ad-status', 'data-ad-type', 'data-ad-unit', 'data-ad-placement'
            ]
            
            has_ad_attribute = False
            for attr in ad_attributes:
                attr_value = element.get_attribute(attr)
                if attr_value and attr_value.strip():
                    self.logger.debug(f"[VALIDATE] Found valid ad attribute: {attr}='{attr_value}'")
                    has_ad_attribute = True
                    break
            
            if not has_ad_attribute:
                self.logger.debug(f"[VALIDATE] No valid ad attributes found")
                return False
            
            # Validasi tambahan: Cek apakah elemen benar-benar dapat diklik
            try:
                # Cek apakah elemen memiliki href atau onclick
                href = element.get_attribute('href')
                onclick = element.get_attribute('onclick')
                
                if not href and not onclick:
                    # Cek apakah elemen memiliki child yang dapat diklik
                    clickable_children = element.find_elements(By.TAG_NAME, 'a')
                    if not clickable_children:
                        self.logger.debug(f"[VALIDATE] Element has no clickable attributes or children")
                        return False
                
                # Cek apakah elemen tidak disabled
                disabled = element.get_attribute('disabled')
                if disabled:
                    self.logger.debug(f"[VALIDATE] Element is disabled")
                    return False
                    
            except Exception as e:
                self.logger.debug(f"[VALIDATE] Error checking clickability: {e}")
                return False
            ...
            ad_keywords = [
                'ad', 'ads', 'advertisement', 'adsbygoogle', 'google-ads',
                'adsense', 'banner', 'sponsor', 'promo', 'commercial',
                'marketing', 'promotion', 'sponsored', 'featured'
            ]
            
            for keyword in ad_keywords:
                if keyword in class_name.lower() or keyword in id_name.lower():
                    self.logger.debug(f"[VALIDATE] Found ad keyword '{keyword}' in class/id")
                    return True
            
            # Check for iframe with ad sources
            if element.tag_name == 'iframe':
                src = element.get_attribute('src') or ''
                if any(ad_domain in src.lower() for ad_domain in ['googleads', 'adsense', 'doubleclick', 'googlesyndication']):
                    self.logger.debug(f"[VALIDATE] Found ad iframe with source: {src}")
                    return True
            
            # Check for Google AdSense specific patterns
            outer_html = element.get_attribute('outerHTML') or ''
            if any(pattern in outer_html.lower() for pattern in ['adsbygoogle', 'google_ads', 'googlesyndication', 'doubleclick']):
                self.logger.debug(f"[VALIDATE] Found AdSense pattern in HTML")
                return True
            
            # More lenient validation for generic ad elements
            if size['width'] > 100 and size['height'] > 50:  # Reasonable size
                # Check if element has clickable content
                try:
                    if element.is_enabled() and element.is_displayed():
                        self.logger.debug(f"[VALIDATE] Generic ad element passed validation")
                        return True
                except:
                    pass
            
            self.logger.debug(f"[VALIDATE] Element failed validation")
            return False
            
        except Exception as e:
            self.logger.debug(f"[VALIDATE] Error validating element: {e}")
            return False
    
    def _is_excluded_ad_element(self, element):
        """Check if element should be excluded based on sample analysis"""
        try:
            # Check for unfilled ads
            ad_status = element.get_attribute('data-ad-status')
            if ad_status == 'unfilled':
                self.logger.debug(f"[EXCLUDE] Unfilled ad detected")
                return True
            
            # Check for hidden ads
            style = element.get_attribute('style') or ''
            if 'display: none' in style or 'height: 0' in style or 'width: 0' in style:
                self.logger.debug(f"[EXCLUDE] Hidden ad detected")
                return True
            
            # Check for auto-placed ads that are unfilled
            class_name = element.get_attribute('class') or ''
            if 'adsbygoogle-noablate' in class_name and ad_status == 'unfilled':
                self.logger.debug(f"[EXCLUDE] Auto-placed unfilled ad detected")
                return True
            
            # Check for ads with opacity 0
            if 'opacity: 0' in style:
                self.logger.debug(f"[EXCLUDE] Invisible ad detected")
                return True
            
            # Check for ads with undefined dimensions
            if 'width: undefined' in style or 'height: undefined' in style:
                self.logger.debug(f"[EXCLUDE] Undefined dimension ad detected")
                return True
            
            return False
            
        except Exception as e:
            self.logger.debug(f"[EXCLUDE] Error checking exclusion: {e}")
            return False
    ...
    def _calculate_click_probability(self, element, ad_type):
        ...
        supported_types = ['google_adsense', 'google_vignette', 'google_discovery']
        ...
        try:
            ...
            if ad_type == 'google_adsense':
                ...
                base_probability *= 1.8  # 80% higher for vignette ads (auto-placed, high value)
            ...
            # Bonus for banner ads (priority focus)
            class_name = element.get_attribute('class') or ''
            id_name = element.get_attribute('id') or ''
            banner_keywords = ['banner', 'header-ad', 'top-ad', 'leaderboard', 'skyscraper', 'rectangle', 'display-ad', 'banner-ad', 'ad-banner']
            
            if any(keyword in class_name.lower() or keyword in id_name.lower() for keyword in banner_keywords):
                base_probability *= 1.5  # 50% bonus for banner ads
                self.logger.debug(f"[BANNER_BONUS] Banner ad detected, applying 1.5x bonus")
    ...
    def click_ads(self, max_clicks=1, conservative=True):
        """Click ads dengan conservative behavior and enhanced debugging"""
        clicked_ads = []
        
        try:
            self.logger.info(f"[CLICK] Starting ad clicking process - Max clicks: {max_clicks}, Conservative: {conservative}")
            
            # Detect ads on current page
            detected_ads = self.detect_ads()
            
            # Enhanced logging for detected ads
            self.logger.info(f"[CLICK] Detected {len(detected_ads)} ads on page")
            for i, ad in enumerate(detected_ads):
                self.logger.info(f"[CLICK] Ad {i+1}: Type={ad['type']}, Visible={ad['is_visible']}, Size={ad['size']['width']}x{ad['size']['height']}, Position=({ad['position']['x']}, {ad['position']['y']}), Prob={ad['click_probability']:.3f}")
            
            if not detected_ads:
                self.logger.info("[CLICK] No ads detected on page")
                return clicked_ads
            ...
            clicks_attempted = 0
            self.logger.info(f"[CLICK] Starting to evaluate {len(detected_ads)} ads for clicking")
            
            for ad_info in detected_ads:
                if clicks_attempted >= max_clicks:
                    break
                
                self.logger.info(f"[CLICK] Evaluating ad: Type={ad_info['type']}, Prob={ad_info['click_probability']:.3f}")
                
                if self._should_click_ad(ad_info, conservative):
                    self.logger.info(f"[CLICK] Attempting to click ad: {ad_info['type']}")
                    if self._click_ad_safely(ad_info):
                        clicked_ads.append(ad_info)
                        clicks_attempted += 1
                        self.logger.info(f"[CLICK] Successfully clicked ad: {ad_info['type']}")
                        ...
                        self.logger.warning(f"[CLICK] Failed to click ad: {ad_info['type']}")
                else:
                    self.logger.info(f"[CLICK] Skipping ad: {ad_info['type']} (probability or conservative check failed)")
            
            self.logger.info(f"[CLICK] Ad clicking completed: {len(clicked_ads)} ads clicked")
            return clicked_ads
            
        except Exception as e:
            self.logger.error(f"[CLICK] Error in ad clicking: {e}")
            return clicked_ads
    ...
    def _click_ad_safely(self, ad_info):
        ...
        try:
            ...
            # Simpan tab handles sebelum klik untuk deteksi tab baru
            self._original_tabs_before_click = self.driver.window_handles
            self._original_tab_handle = self.driver.current_window_handle
            self.logger.debug(f"[TAB_TRACKING] Saved {len(self._original_tabs_before_click)} tabs before click")
            
            # Use mouse simulator for human-like clicking
            if self.mouse.move_to_element(element):
                # Simpan URL sebelum klik untuk validasi
                url_before_click = self.driver.current_url
                self.logger.info(f"[CLICK] URL before click: {url_before_click[:60]}...")
                
                # Lakukan klik
                element.click()
                self.logger.info(f"[CLICK] Clicked element with selector: {ad_info['selector']}")
                
                # Tunggu sebentar untuk navigasi
                time.sleep(2)
                
                # Validasi apakah benar-benar terjadi navigasi
                url_after_click = self.driver.current_url
                self.logger.info(f"[CLICK] URL after click: {url_after_click[:60]}...")
                
                # Cek apakah URL berubah (navigasi terjadi)
                if url_before_click == url_after_click:
                    self.logger.warning(f"[CLICK] No navigation occurred - click may have failed")
                    # Cek apakah ada tab baru yang dibuka
                    current_tabs = self.driver.window_handles
                    if len(current_tabs) > len(self._original_tabs_before_click):
                        self.logger.info(f"[CLICK] New tab opened instead of navigation")
                    else:
                        self.logger.warning(f"[CLICK] Click appears to have failed - no navigation or new tab")
                        # Jangan lanjutkan ke _handle_ad_landing_page jika tidak ada navigasi
                        return False
                else:
                    # Validasi tambahan: Pastikan navigasi adalah ke domain yang berbeda
                    from urllib.parse import urlparse
                    try:
                        before_domain = urlparse(url_before_click).netloc
                        after_domain = urlparse(url_after_click).netloc
                        
                        if before_domain == after_domain:
                            self.logger.warning(f"[CLICK] Navigation within same domain - may not be ad click")
                            # Tetap lanjutkan tetapi dengan peringatan
                        else:
                            self.logger.info(f"[CLICK] Navigation to different domain: {before_domain} -> {after_domain}")
                    except Exception as e:
                        self.logger.debug(f"[CLICK] Error parsing domains: {e}")
                ...
                # Special handling for vignette ads (auto-placed behavior)
    ...
    def _check_invalid_traffic_protection(self):
        ...
        try:
            ...
            min_session_duration = self.config.get('ad_clicking.min_session_duration', 5)  # Reduced to 5 seconds
            ...
            # Check natural bounce rate (reduced for banner ads)
            natural_bounce_rate = self.config.get('ad_clicking.natural_bounce_rate', 0.3)  # Reduced from 0.5 to 0.3
    ...
    def _handle_ad_landing_page(self):
        """Handle ad landing page after click with tab detection"""
        try:
            # Tunggu sebentar untuk navigasi
            time.sleep(2)
            
            # Periksa apakah tab handles tersedia
            if not hasattr(self, '_original_tabs_before_click') or not self._original_tabs_before_click:
                self.logger.warning("[LANDING] No tab tracking data available, using fallback method")
            ...
            # Periksa apakah ada tab baru
            current_tabs = self.driver.window_handles
            new_tabs = [tab for tab in current_tabs if tab not in self._original_tabs_before_click]
            
            self.logger.debug(f"[TAB_TRACKING] Original tabs: {len(self._original_tabs_before_click)}, Current tabs: {len(current_tabs)}, New tabs: {len(new_tabs)}")
            
            if new_tabs:
                # Iklan dibuka di tab baru
                self.logger.info("[LANDING] Ad opened in new tab")
                new_tab = new_tabs[0]
                self.driver.switch_to.window(new_tab)
                
                current_url = self.driver.current_url
                self.logger.info(f"[LANDING] New tab URL: {current_url[:60]}...")
                
                # Simulate behavior di tab baru
                self._simulate_landing_page_behavior()
                
                # Tutup tab baru dan kembali ke tab asli
                self.driver.close()
                self.driver.switch_to.window(self._original_tab_handle)
                self.logger.info("[RETURN] Closed new tab and returned to original tab")
                
            else:
                # Iklan dibuka di halaman yang sama
                self.logger.info("[LANDING] Ad opened in same page")
                current_url = self.driver.current_url
                self.logger.info(f"[LANDING] Navigated to: {current_url[:60]}...")
                
                # Simulate behavior di halaman yang sama
                self._simulate_landing_page_behavior()
            ...
            self._original_tabs_before_click = None
            self._original_tab_handle = None
    ...
    def _handle_vignette_ad_behavior(self):
        ...
        try:
            ...
            try:
                # Look for vignette-specific elements using DetectionPatterns config
                vignette_selectors = self.patterns.ad_detection_patterns.get('google_vignette', [])
                vignette_elements = []
                
                for selector in vignette_selectors:
                    elements = self.driver.find_elements(By.CSS_SELECTOR, selector)
                    vignette_elements.extend(elements)
                ...
                if vignette_elements:
                    ...
                    if random.random() < 0.7:  # 70% chance to continue
                        ...
                        self.logger.info("[VIGNETTE] Simulating vignette ad close by clicking outside")
                        # Click outside the vignette area to dismiss it
                        self._click_outside_vignette_to_dismiss()
                else:
    ...
    def _click_outside_vignette_to_dismiss(self):
        """Click outside the vignette area to dismiss it"""
        try:
            ...
            # Calculate safe outside click positions (avoid center area where vignette might be)
            # Click in one of the corners or edges
            click_positions = [
                ...
                (viewport_width // 2, 30),  # Top edge
                (viewport_width // 2, viewport_height - 30),  # Bottom edge
                (30, viewport_height // 2),  # Left edge
                (viewport_width - 30, viewport_height // 2),  # Right edge
            ]
            
            # Choose a random position
            x, y = random.choice(click_positions)
            
            self.logger.info(f"[VIGNETTE] Clicking outside vignette at position ({x}, {y})")
            
            # Move mouse to the position and click
            self.mouse.human_like_movement((0, 0), (x, y))
            ...
            # Wait a bit for the vignette to dismiss
            time.sleep(random.uniform(1.0, 2.0))
            
        except Exception as e:
            self.logger.warning(f"[VIGNETTE] Error clicking outside to dismiss: {e}")
            # Fallback: just wait
            time.sleep(random.uniform(2.0, 4.0))
...
class PostNavigator:
    ...
    def navigate_to_post(self, link_element, post_type="unknown"):
        """Navigate to a post link with natural behavior and robust error handling"""
        try:
            if not link_element:
                return False
            ...
            # Scroll to link with better positioning
            self.driver.execute_script("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", link_element)
            time.sleep(2)  # Increased wait time for better positioning
            
            # Check if element is visible and clickable
            if not self._is_element_clickable(link_element):
                self.logger.warning(f"[WARNING] Element not clickable, trying alternative methods")
                return self._navigate_with_alternative_methods(link_element, post_type, href)
            ...
            click_success = self._try_multiple_click_strategies(link_element, post_type)
            ...
            if click_success:
                ...
                self.logger.error(f"[ERROR] All click strategies failed for {post_type} post")
                return False
    ...
    def _is_element_clickable(self, element):
        """Check if element is visible and clickable with comprehensive overlap detection"""
        try:
            # Check if element is displayed
            if not element.is_displayed():
                self.logger.warning("[CHECK] Element is not displayed")
                return False
            
            # Check if element is enabled
            if not element.is_enabled():
                self.logger.warning("[CHECK] Element is not enabled")
                return False
            ...
            if not is_in_viewport:
                self.logger.warning("[CHECK] Element is not in viewport")
                return False
            
            # Enhanced overlap detection - check multiple points
            overlap_info = self.driver.execute_script(
                "var element = arguments[0];"
                "var rect = element.getBoundingClientRect();"
                "var centerX = rect.left + rect.width / 2;"
                "var centerY = rect.top + rect.height / 2;"
                "var topLeftX = rect.left + 5;"
                "var topLeftY = rect.top + 5;"
                "var bottomRightX = rect.right - 5;"
                "var bottomRightY = rect.bottom - 5;"
                ""
                "var centerElement = document.elementFromPoint(centerX, centerY);"
                "var topLeftElement = document.elementFromPoint(topLeftX, topLeftY);"
                "var bottomRightElement = document.elementFromPoint(bottomRightX, bottomRightY);"
                ""
                "var isCenterCovered = centerElement !== element;"
                "var isTopLeftCovered = topLeftElement !== element;"
                "var isBottomRightCovered = bottomRightElement !== element;"
                ""
                "return {"
                "  isCenterCovered: isCenterCovered,"
                "  isTopLeftCovered: isTopLeftCovered,"
                "  isBottomRightCovered: isBottomRightCovered,"
                "  centerElement: centerElement ? centerElement.tagName + (centerElement.className ? '.' + centerElement.className : '') : 'null',"
                "  topLeftElement: topLeftElement ? topLeftElement.tagName + (topLeftElement.className ? '.' + topLeftElement.className : '') : 'null',"
                "  bottomRightElement: bottomRightElement ? bottomRightElement.tagName + (bottomRightElement.className ? '.' + bottomRightElement.className : '') : 'null'"
                "};",
                element
            )
            
            if overlap_info['isCenterCovered'] or overlap_info['isTopLeftCovered'] or overlap_info['isBottomRightCovered']:
                self.logger.warning(f"[CHECK] Element is covered by other elements:")
                self.logger.warning(f"  Center covered by: {overlap_info['centerElement']}")
                self.logger.warning(f"  Top-left covered by: {overlap_info['topLeftElement']}")
                self.logger.warning(f"  Bottom-right covered by: {overlap_info['bottomRightElement']}")
                return False
            
            # Check if element has proper z-index and positioning
            positioning_info = self.driver.execute_script(
                "var element = arguments[0];"
            ...
            self.logger.debug(f"[CHECK] Element positioning: z-index={positioning_info['zIndex']}, "
                            f"position={positioning_info['position']}, "
                            f"opacity={positioning_info['opacity']}")
            
            # Check if element is too small (might be hidden)
            if positioning_info['width'] < 10 or positioning_info['height'] < 10:
                self.logger.warning(f"[CHECK] Element is too small: {positioning_info['width']}x{positioning_info['height']}")
                return False
            
            return True
            
        except Exception as e:
            self.logger.warning(f"[CHECK] Error checking element clickability: {e}")
            return False
    
    def _try_multiple_click_strategies(self, element, post_type):
        """Try multiple click strategies to handle element click intercepted errors"""
        strategies = [
            ("Direct Click", lambda: element.click()),
            ("JavaScript Click", lambda: self.driver.execute_script("arguments[0].click();", element)),
            ("ActionChains Click", lambda: self.driver.execute_script(
                "var event = new MouseEvent('click', {bubbles: true, cancelable: true});"
                "arguments[0].dispatchEvent(event);", element
            )),
            ("Force Click with JS", lambda: self.driver.execute_script(
                "arguments[0].style.pointerEvents = 'auto';"
                "arguments[0].click();", element
            )),
            ("Scroll and Click", lambda: self._scroll_and_click(element)),
            ("Hide Overlapping Elements", lambda: self._hide_overlapping_elements_and_click(element)),
            ("Reposition Element", lambda: self._reposition_element_and_click(element)),
            ("Force Z-Index", lambda: self._force_z_index_and_click(element))
        ]
        
        for strategy_name, strategy_func in strategies:
            try:
                self.logger.info(f"[STRATEGY] Trying {strategy_name}")
                strategy_func()
                self.logger.info(f"[SUCCESS] {strategy_name} succeeded for {post_type} post")
                return True
            except Exception as e:
    ...
    def _scroll_and_click(self, element):
        """Scroll element to center and click"""
        ...
        # Try to remove any overlapping elements temporarily
        self.driver.execute_script(
            "var element = arguments[0];"
            "var rect = element.getBoundingClientRect();"
            "var centerX = rect.left + rect.width / 2;"
            ...
            "  setTimeout(function() { elementAtPoint.style.pointerEvents = 'auto'; }, 1000);"
            "}",
            element
        )
        time.sleep(0.5)
    ...
    def _hide_overlapping_elements_and_click(self, element):
        """Hide overlapping elements temporarily and click"""
        try:
            # Find and hide overlapping elements
            self.driver.execute_script("""
                ...
                // Find all elements at the center point
                var elementAtPoint = document.elementFromPoint(centerX, centerY);
                var hiddenElements = [];
                
                // Hide overlapping elements (but not the target)
                while (elementAtPoint && elementAtPoint !== targetElement && elementAtPoint !== document.body) {
                    if (elementAtPoint.style) {
                        elementAtPoint.style.visibility = 'hidden';
                        hiddenElements.push(elementAtPoint);
                    }
                    elementAtPoint = document.elementFromPoint(centerX, centerY);
                }
                
                // Store hidden elements for restoration
                window._hiddenElements = hiddenElements;
            """, element)
            
            time.sleep(0.5)
            element.click()
            
            # Restore hidden elements after click
            self.driver.execute_script("""
                if (window._hiddenElements) {
                    window._hiddenElements.forEach(function(el) {
                        el.style.visibility = '';
                    });
                    delete window._hiddenElements;
                }
            """)
            
        except Exception as e:
            self.logger.warning(f"[HIDE_OVERLAP] Failed to hide overlapping elements: {e}")
            raise
    
    def _reposition_element_and_click(self, element):
        """Reposition element to a safe location and click"""
        try:
            self.driver.execute_script("""
                var element = arguments[0];
                var originalStyle = {
                ...
                element._originalStyle = originalStyle;
            ...
            # Restore original position after click
            self.driver.execute_script("""
                var element = arguments[0];
                if (element._originalStyle) {
                    element.style.position = element._originalStyle.position;
                    element.style.top = element._originalStyle.top;
                    element.style.left = element._originalStyle.left;
                    element.style.zIndex = element._originalStyle.zIndex;
                    element.style.transform = element._originalStyle.transform;
                    delete element._originalStyle;
                }
            """, element)
            
        except Exception as e:
            self.logger.warning(f"[REPOSITION] Failed to reposition element: {e}")
            raise
    
    def _force_z_index_and_click(self, element):
        """Force higher z-index and click"""
        try:
            self.driver.execute_script("""
                var element = arguments[0];
                ...
                element._originalZIndex = originalZIndex;
            """, element)
            ...
            self.driver.execute_script("""
                var element = arguments[0];
                if (element._originalZIndex !== undefined) {
            ...
            self.logger.warning(f"[FORCE_ZINDEX] Failed to force z-index: {e}")
            raise

    def _navigate_with_alternative_methods(self, element, post_type, href):
        """Try alternative navigation methods when direct clicking fails"""
        try:
            self.logger.info(f"[ALTERNATIVE] Trying alternative navigation for {post_type} post")
            
            # Method 1: Direct URL navigation
            if href:
                self.logger.info("[ALTERNATIVE] Method 1: Direct URL navigation")
                self.driver.get(href)
                time.sleep(3)
                self.logger.info(f"[SUCCESS] Successfully navigated to {post_type} post via direct URL")
                return True
            
            # Method 2: Keyboard navigation (if element is focusable)
            try:
                self.logger.info("[ALTERNATIVE] Method 2: Keyboard navigation")
                element.send_keys("\n")  # Press Enter
                time.sleep(3)
                self.logger.info(f"[SUCCESS] Successfully navigated to {post_type} post via keyboard")
                return True
            except:
                pass
            
            # Method 3: Right-click and open in new tab, then switch
            try:
                self.logger.info("[ALTERNATIVE] Method 3: Right-click navigation")
                try:
                    from selenium.webdriver.common.action_chains import ActionChains
                    ...
                    # Try to find and click "Open in new tab" option
                    # This is browser-specific and might not work in all cases
                    self.logger.warning("[ALTERNATIVE] Right-click method not fully implemented")
                except ImportError:
                    self.logger.warning("[ALTERNATIVE] ActionChains not available")
            except:
                pass
            ...
            self.logger.error(f"[ALTERNATIVE] All alternative methods failed: {e}")
            return False
...
class ArticleBrowser:
    ...
    def __init__(self, driver, timing_system, mouse_simulator, risk_monitor):
        ...
        # Mix mode statistics tracking
        self.mix_stats = {
            'direct_navigations': 0,
            'referer_navigations': 0,
            'total_navigations': 0,
            'session_start_time': time.time(),
            'natural_pattern_adjustments': 0
        }
    ...
    def open_article_with_referer(self, article_url=None, referer_url=None):
        ...
        """
        # Check configuration
        use_referer = self.get_use_referer()
        referer_method = self.get_referer_method()
        mix_enabled = self.get_mix_direct_referer()
        direct_ratio = self.get_direct_ratio()
        natural_pattern = self.get_natural_pattern()
        ...
        # Smart navigation decision based on configuration
        if mix_enabled:
            # Mix mode: Decide between direct and referer based on ratio
            success = self._smart_mix_navigation(article_url, referer_url, direct_ratio, natural_pattern)
        elif not use_referer:
            # Direct mode only
            self.logger.info("[CONFIG] Referer is disabled, using direct navigation")
            success = self._direct_navigation_flow(article_url)
        else:
            # Referer mode only
            success = self._referer_only_navigation(article_url, referer_url, referer_method)
    ...
    def set_use_referer(self, use_referer):
        """Set whether to use referer or direct navigation"""
        self.config.config['article']['use_referer'] = use_referer
        self.logger.info(f"Referer usage set to: {use_referer}")
    
    def get_use_referer(self):
        """Get whether referer is enabled"""
        return self.config.get('article.use_referer', True)
    
    def set_referer_method(self, method):
        """Set the referer method to use"""
        valid_methods = ['auto', 'real', 'proxy', 'cdp', 'direct']
        if method not in valid_methods:
            raise ValueError(f"Method must be one of: {valid_methods}")
        self.config.config['article']['referer_method'] = method
        self.logger.info(f"Referer method set to: {method}")
    
    def get_referer_method(self):
        """Get the current referer method"""
        return self.config.get('article.referer_method', 'auto')
    
    def set_mix_direct_referer(self, enabled):
        """Enable/disable mix of direct and referer traffic"""
        self.config.config['article']['mix_direct_referer'] = enabled
        self.logger.info(f"Mix direct-referer traffic set to: {enabled}")
    
    def get_mix_direct_referer(self):
        """Get whether mix direct-referer is enabled"""
        return self.config.get('article.mix_direct_referer', False)
    
    def set_direct_ratio(self, ratio):
        """Set the ratio of direct traffic (0.0-1.0)"""
        if not 0.0 <= ratio <= 1.0:
            raise ValueError("Direct ratio must be between 0.0 and 1.0")
        self.config.config['article']['direct_ratio'] = ratio
        self.logger.info(f"Direct traffic ratio set to: {ratio*100:.1f}%")
    
    def get_direct_ratio(self):
        """Get the current direct traffic ratio"""
        return self.config.get('article.direct_ratio', 0.3)
    
    def set_natural_pattern(self, enabled):
        """Enable/disable natural traffic patterns"""
        self.config.config['article']['natural_pattern'] = enabled
        self.logger.info(f"Natural traffic pattern set to: {enabled}")
    
    def get_natural_pattern(self):
        """Get whether natural pattern is enabled"""
        return self.config.get('article.natural_pattern', True)
    ...
    def _direct_navigation_flow(self, article_url):
        """Enhanced direct navigation flow with natural simulation"""
        try:
            self.logger.info(f"[DIRECT] Starting enhanced direct navigation to: {article_url}")
            
            # Direct mode - no special flags needed, use same logic as referer
            
            # Step 1: Direct navigation with natural delay
            self.logger.info("[DIRECT] Step 1: Navigating to article with natural delay")
            time.sleep(random.uniform(1, 3))  # Natural delay before navigation
            self.driver.get(article_url)
            
            # Step 2: Wait for page to load with natural behavior
            self.logger.info("[DIRECT] Step 2: Waiting for article page to load")
            time.sleep(random.uniform(3, 6))  # Natural loading time
            
            # Step 3: Verify navigation success
            current_url = self.driver.current_url
            if article_url not in current_url:
                self.logger.error(f"[ERROR] Direct navigation failed. Current URL: {current_url}")
                return False
            
            self.logger.info("[DIRECT] Step 3: Navigation successful, simulating natural behavior")
            
            # Step 4: Use SAME logic as referer mode (no duplication!)
            self.logger.info("[DIRECT] Step 4: Using referer browsing logic (reuse existing method)")
            self._simulate_referer_browsing()
            
            # Step 6: Final verification and stabilization
            self.logger.info("[DIRECT] Step 5: Final page stabilization")
            time.sleep(random.uniform(2, 4))
            
            self.logger.info("[SUCCESS] Enhanced direct navigation completed successfully")
            return True
                
        except Exception as e:
            self.logger.error(f"[ERROR] Enhanced direct navigation failed: {e}")
            return False
        finally:
            # Direct mode completed - no special cleanup needed
            pass
    
    def _auto_referer_flow(self, article_url, referer_url):
        """Auto referer flow with fallback mechanism"""
        success = False
        
        try:
            self.logger.info("[AUTO] Starting auto referer flow with fallback mechanism")
        ...
        if not success:
            try:
                ...
                self.logger.error(f"Direct navigation fallback failed: {e}")
        
        return success
    
    def _smart_mix_navigation(self, article_url, referer_url, direct_ratio, natural_pattern):
        """Smart mix navigation with natural patterns"""
        try:
            # Determine navigation type based on ratio and natural patterns
            use_direct = self._should_use_direct_navigation(direct_ratio, natural_pattern)
            
            if use_direct:
                self.logger.info(f"[MIX] Using direct navigation ({direct_ratio*100:.1f}% ratio)")
                self._track_navigation_type('direct')
                return self._direct_navigation_flow(article_url)
            else:
                self.logger.info(f"[MIX] Using referer navigation ({(1-direct_ratio)*100:.1f}% ratio)")
                self._track_navigation_type('referer')
                
                # Get referer URL if not provided
                if referer_url is None:
                    high_cpc_weight = self.get_high_cpc_weight()
                    referer_url = self.get_weighted_referer_url()
                    self.logger.info(f"[MIX] Weighted selected referer URL ({high_cpc_weight*100:.0f}% high CPC): {referer_url}")
                
                # Use auto referer flow with fallback
                return self._auto_referer_flow(article_url, referer_url)
                
        except Exception as e:
            self.logger.error(f"[ERROR] Smart mix navigation failed: {e}")
            # Fallback to direct navigation
            self._track_navigation_type('direct')
            return self._direct_navigation_flow(article_url)
    
    def _should_use_direct_navigation(self, direct_ratio, natural_pattern):
        """Determine if should use direct navigation based on ratio and natural patterns"""
        if natural_pattern:
            # Natural pattern: Add some randomness and session-based logic
            base_ratio = direct_ratio
            
            # Add session-based variation (more direct at start, more referer later)
            ...
            self.logger.info(f"[MIX] Natural pattern - Base: {base_ratio:.2f}, Session: {session_factor:.2f}, Final: {final_ratio:.2f}")
            return random.random() < final_ratio
        else:
            # Simple ratio-based decision
            return random.random() < direct_ratio
    
    def _get_session_factor(self):
        """Get session-based factor for natural patterns"""
        # More direct traffic at the beginning of session, more referer later
        if not hasattr(self, '_session_start_time'):
            self._session_start_time = time.time()
        
        session_duration = time.time() - self._session_start_time
        
        # Start with higher direct ratio, decrease over time
        if session_duration < 60:  # First minute
            return 1.2  # 20% more direct
        elif session_duration < 300:  # First 5 minutes
            return 1.0  # Normal ratio
        else:  # After 5 minutes
            return 0.8  # 20% less direct (more referer)
    
    def _referer_only_navigation(self, article_url, referer_url, referer_method):
        """Referer-only navigation based on method"""
        # Get referer URL if not provided
        if referer_url is None:
            high_cpc_weight = self.get_high_cpc_weight()
            referer_url = self.get_weighted_referer_url()
            self.logger.info(f"[REFERER] Weighted selected referer URL ({high_cpc_weight*100:.0f}% high CPC): {referer_url}")
        
        self.logger.info(f"Opening article: {article_url} with referer: {referer_url}")
        
        # Determine which referer method to use
        if referer_method == 'direct':
            self.logger.info("[CONFIG] Referer method set to 'direct', using direct navigation")
            return self._direct_navigation_flow(article_url)
        elif referer_method == 'auto':
            return self._auto_referer_flow(article_url, referer_url)
        elif referer_method == 'real':
            return self._real_referer_flow(article_url, referer_url)
        elif referer_method == 'proxy':
            return self._proxy_referer_flow(article_url, referer_url)
        elif referer_method == 'cdp':
            return self._cdp_referer_flow(article_url, referer_url)
        else:
            self.logger.warning(f"Unknown referer method: {referer_method}, falling back to auto")
            return self._auto_referer_flow(article_url, referer_url)
    
    # REMOVED: All duplicate methods - using existing _simulate_referer_browsing() instead
    
    def _track_navigation_type(self, nav_type):
        """Track navigation type for statistics"""
        self.mix_stats['total_navigations'] += 1
        if nav_type == 'direct':
            self.mix_stats['direct_navigations'] += 1
        elif nav_type == 'referer':
            self.mix_stats['referer_navigations'] += 1
    
    def get_mix_statistics(self):
        """Get mix mode statistics"""
        total = self.mix_stats['total_navigations']
        if total == 0:
            return {
                'total_navigations': 0,
                'direct_percentage': 0,
                'referer_percentage': 0,
                'session_duration': 0,
                'natural_adjustments': 0
            }
        
        direct_pct = (self.mix_stats['direct_navigations'] / total) * 100
        referer_pct = (self.mix_stats['referer_navigations'] / total) * 100
        session_duration = time.time() - self.mix_stats['session_start_time']
        
        return {
            'total_navigations': total,
            'direct_navigations': self.mix_stats['direct_navigations'],
            'referer_navigations': self.mix_stats['referer_navigations'],
            'direct_percentage': round(direct_pct, 1),
            'referer_percentage': round(referer_pct, 1),
            'session_duration_minutes': round(session_duration / 60, 1),
            'natural_adjustments': self.mix_stats['natural_pattern_adjustments']
        }
    
    def reset_mix_statistics(self):
        """Reset mix statistics"""
        self.mix_stats = {
            'direct_navigations': 0,
            'referer_navigations': 0,
            'total_navigations': 0,
        ...
        self.logger.info("[MIX] Statistics reset")
    ...
    def run_advanced_automation(self):
        ...
        try:
            ...
            if self.process_advanced_features:
                ...
                # NEW: Configure referer usage
                # article_browser.set_use_referer(True)  # Enable/disable referer (default: True)
                # article_browser.set_referer_method('auto')  # 'auto', 'real', 'proxy', 'cdp', 'direct'
                
                # Example configurations:
                # article_browser.set_use_referer(False)  # Use direct navigation only
                # article_browser.set_referer_method('direct')  # Force direct navigation
                # article_browser.set_referer_method('real')  # Use only real referer flow
                # article_browser.set_referer_method('auto')  # Auto with fallback (default)
                
                # NEW: Mix Direct + Referer Configuration
                # article_browser.set_mix_direct_referer(True)  # Enable mix mode
                # article_browser.set_direct_ratio(0.3)  # 30% direct, 70% referer
                # article_browser.set_natural_pattern(True)  # Use natural traffic patterns
                
                # Mix mode examples:
                # article_browser.set_mix_direct_referer(True)  # Enable mix mode
                # article_browser.set_direct_ratio(0.2)  # 20% direct, 80% referer (more AdSense revenue)
                # article_browser.set_direct_ratio(0.5)  # 50% direct, 50% referer (balanced)
                # article_browser.set_direct_ratio(0.7)  # 70% direct, 30% referer (more direct)
                ...
                if success:
                    ...
                    # Get mix statistics if mix mode is enabled
                    if self.article_browser.get_mix_direct_referer():
                        mix_stats = self.article_browser.get_mix_statistics()
                        logging.info(f"[MIX STATS] Direct: {mix_stats['direct_percentage']}%, Referer: {mix_stats['referer_percentage']}%")
                        logging.info(f"[MIX STATS] Total navigations: {mix_stats['total_navigations']}, Session: {mix_stats['session_duration_minutes']} min")
